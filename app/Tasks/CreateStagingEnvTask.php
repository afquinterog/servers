...
class CreateStagingEnvTask extends GenericTask
{
    ...
    public function run($server, $task)
    {
         ...
         //Get cloud provider
         $this->provider = resolve('App\Cloud\GenericCloudProvider');

        //Get server parameters
        $schema = $server->getParameter("SCHEMA");
        $partnerName = $server->getParameter("PARTNER_NAME");
        $domain = $server->getParameter("DOMAIN");
        $syndicationTenantId = $server->getParameter("SYNDICATION_TENANT_ID");
        $syndicationAuthToken = $server->getParameter("SYNDICATION_AUTH_TOKEN");
        $uiBranch = $server->getParameter("UI_BRANCH");

        if( $server->code == ""){

            $parameters = $this->provider->getInstanceCreationParameters($server);
            $result = $this->provider->createInstance($parameters);
            $instanceId = $this->provider->extractValue($result, 'InstanceId');
            ...
            $apiResult = $this->configureApi($server, $schema,  $partnerName, $syndicationTenantId, $syndicationAuthToken);
    
            $uiResult = $this->configureUi($server, $domain, $uiBranch);
